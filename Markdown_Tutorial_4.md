...
缩进的代码块以4个或者4个以上的空格开始，表示这行是一个代码块。当你想使用一个缩进代码块时，缩进代码块前和上面的段落文本之间需要有空行来分隔。缩进代码块中，这行4个空格缩进之后的内容都被视为代码内容，代码内容里的Markdown标识符不会被解析，任何超过四个的初始空格都将包含在内容中，即使是内部空白行。
...
有时文本中带块结构会自带段落的中断（比如分隔线、标题等结构），这些块结构后可以直接接缩进代码块，不需要空白行做段落分隔。
